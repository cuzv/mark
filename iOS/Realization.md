...
[方便快捷获取调整视图坐标于大小](#方便快捷获取调整视图坐标于大小)
[为视图添加边线](#为视图添加边线)
[寻找视图的控制器](#寻找视图的控制器)
[吐司框效果](#吐司框效果)
[徽标效果](#徽标效果)
...
```objective-c
#define kTextLimit 100
- (void)textViewDidBeginEditing:(UITextView *)textView {
    if ([textView.text isEqualToString:kTextPlaceHolder]) {
        textView.textColor = DarkElementColor;
        textView.text = @"";
        _countLabel.text = [@(kTextLimit) stringValue];
    }
}

- (void)textViewDidEndEditing:(UITextView *)textView {
    if (textView.text.length < 1) {
        textView.text = kTextPlaceHolder;
        textView.textColor = ElementCColor;
    }
}

- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (NSMaxRange(range) + text.length > kTextLimit){
        return NO;
    }
    if (text.length > 0){
        if(_countLabel.text.intValue > 0){
            _countLabel.text = [NSString stringWithFormat:@"%d", kTextLimit - (NSMaxRange(range) + text.length)];
            if(textView.text.length == kTextLimit - 1) {
                textView.text = [textView.text stringByAppendingString:text];
            }
        }
    } else {
        _countLabel.text = [NSString stringWithFormat:@"%d", kTextLimit - (textView.text.length - range.length)];
    }
    if (_countLabel.text.intValue <= 0) {
        return NO;
    }
